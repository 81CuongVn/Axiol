...
class TypeRacer:
    ...
    def calculate_result(start_time, end_time, user_content, raw_text):
        ...
        accuracy = round(
            difflib.SequenceMatcher(None, text, user_content).ratio(), 2
        ) * 100
    ...
    async def coro(self, player):
        try:
            ...
            await player.send(
                "Time is up! You failed to complete the test in time."
            )
...
class Fun(commands.Cog):
    ...
    @commands.group(
        name="typeracer", pass_context=True, invoke_without_command=True
    )
    ...
    async def new(self, ctx, player_amount=None):
        ...
        if not player_amount.isnumeric():
            return await ctx.send(
                "The argument which you entered is not numeric.")
    ...
    @has_command_permission()
    async def typing_test(self, ctx, test_type=None):
        if test_type is None:
            return await ctx.send(
                embed=discord.Embed(
                    ...
                    name="Format",
                    value=(
                        f"`{await get_prefix(ctx)}typingtest <type>`\n"
            ...
            return await ctx.send(
                embed=discord.Embed(
                    ...
                    name="time",
                    value=(
                        "Time based typing test, here you have to type as "
                        "much as you can from the text given under the "
                        "time you choose."
                    )
                ).add_field(
                    name="word",
                    value=(
                        "Word base typing test, here you need to complete "
                        "the entire text given and the max time is 60 seconds."
                    )
                )
            )
        ...
        if test_type == "time":
            bot_msg = await ctx.send(
                embed=discord.Embed(
                    ...
                    description=(
                        "Let's see how fast you can type! React to the "
                        f"respective emoji below to start.\n\n{TYPE_15} "
                    ...
                    name="Note",
                    value=(
                        "The task is to type as much as you can in the time"
                        " specified, not to complete all the text given. I will"
                        " inform you when less time is left!"
                    )
                )
            )

            await bot_msg.add_reaction(TYPE_15)
            await bot_msg.add_reaction(TYPE_30)
            await bot_msg.add_reaction(TYPE_60)
            ...
            bot_msg = await ctx.send(
                f"You have selected **{config['time']}** "
        ...
        with BytesIO() as image_binary:
            ...
            try:
                ...
                while not waiter.done():
                    ...
                    if timer == 5:
                        await ctx.send(
                            f"Only 5 seconds left {ctx.author.mention}!")
                    ...
                    raw_wpm = round(
                        (len(user_content) / 5 / time_taken) * 60, 2
                    )
                    ...
                    embed.add_field(
                        name="Raw WPM", value=f"{raw_wpm}", inline=False
                    )
                    ...
                    embed.add_field(
                        name="Accuracy", value=f"{accuracy}%", inline=False
                    )

                    embed.add_field(
                        name="Mistakes", value=mistake_ratio, inline=False
                    )
            ...
            if str(reaction.emoji) == "🇮":
                while True:
                    ...
                    if user_msg.content in [
                        ...
                        await ctx.send("Look the message you sent is not a file or link, try again.")
                    try:
                        await edit.clear_reaction("🇮")
